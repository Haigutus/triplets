# -------------------------------------------------------------------------------
# Name:        RDF parser - pygixml tuned variant
# Purpose:     Uses pygixml with PARSE_MINIMAL | PARSE_EMBED_PCDATA flags
#              for maximum parse speed. Skips escape processing, EOL
#              normalization, CDATA, and attribute whitespace conversion.
#
# Based on:    rdf_parser.py by kristjan.vilgo
# Variant:     pygixml with tuned parse flags (benchmark variant)
# -------------------------------------------------------------------------------
from io import BytesIO
import pygixml

# Tuned parse flags for CIM/RDF XML:
# PARSE_MINIMAL: skip comments, PI, CDATA, escape processing, EOL normalization
# PARSE_EMBED_PCDATA: embed text in parent node (fewer memory allocations)
_PARSE_FLAGS = pygixml.PARSE_MINIMAL | pygixml.PARSE_EMBED_PCDATA
...
def load_RDF_to_list(path_or_fileobject, debug=False, keep_ns=False):
    """Parse a single RDF XML file into a triplestore list using pygixml."""
    ...
    if isinstance(path_or_fileobject, str):
        doc.load_file(path_or_fileobject, _PARSE_FLAGS)
    else:
        path_or_fileobject.seek(0)
        content = path_or_fileobject.read()
        if isinstance(content, bytes):
            doc.load_buffer(content, _PARSE_FLAGS)
        else:
            doc.load_string(content, _PARSE_FLAGS)

    if debug:
        _, start_time = _print_duration("pygixml: XML loaded", start_time)
    ...
    # Extract namespace map from root attributes (xmlns:prefix="uri")
    has_xml_base = False
    ...
    while attr:
        ...
        try:
            ...
            break
        if attr is None or (hasattr(attr, 'is_null') and attr.is_null()):
            break
    ...
    while rdf_object and not rdf_object.is_null():
        ...
        while element and not element.is_null():
            KEY = _split_prefixed_name(element.name)
            VALUE = element.child_value() or None
    ...
    if debug:
        _print_duration("pygixml: All values extracted", start_time)
    return data_list
...
def load_RDF_to_dataframe(path_or_fileobject, debug=False, data_type="string"):
    """Parse a single RDF XML file into a Pandas DataFrame using pygixml."""
...
def load_all_to_dataframe(list_of_paths_to_zip_globalzip_xml, debug=False, data_type="string", max_workers=None):
    """Parse multiple RDF XML files using pygixml into a single Pandas DataFrame."""
    if debug:
