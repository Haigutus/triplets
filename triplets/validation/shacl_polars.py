...
class _Context(SchemaKind):
    ...
    def __init__(self, frame, rdf_map=None):
        ...
        if rdf_map is not None:
            self._class_ids, extra = self._with_ancestors(rdf_map)
            if extra is not None:
                self.membership = polars.concat([self.membership, extra.lazy()])
    ...
    def _with_ancestors(self, rdf_map):
        ...
        for name, parts in expanded.items():
            ...
            if name not in self._class_ids and len(class_ids[name]):
                extra_frames.append(polars.DataFrame({
                    "CLASS": [name] * len(class_ids[name]), "ID": class_ids[name],
                }))
        extra = polars.concat(extra_frames) if extra_frames else None
        return class_ids, extra
